...
> - 🔄 Frontend moving off direct supabase-js calls to explicit backend actions (`<domain>_crud`), page by page. Done: logs / LLM config / API integrations / agents (editor, skills, knowledge gaps, workflow agent pickers) / home overview + sidebar provider dots. Remaining: data, tasks, staff, messages, documents, KB / workflow / automation pages, file storage
